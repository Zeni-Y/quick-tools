...
async function showPreview(origPage, idx) {
  ...
  const page = await pdfDoc.getPage(origPage);
  // トリムモード時は常に rotation=0 で描画（PDF座標系との対応を保つため）
  ...
  if (trimMode) {
    ...
    trimPreviewInfo = { origPage, pageWidthPts: vpNative.width, pageHeightPts: vpNative.height };
    ...
    label.textContent = 'トリムプレビュー: ページ ' + (idx + 1) + ' / ' + activePages.length + ' (元の向き)';
  } else {
  ...
  try {
    ...
    for (let i = 0; i < copiedPages.length; i++) {
      ...
      if (trim) {
        ...
        page.setCropBox(cropBox.x + trim.relX, cropBox.y + trim.relY, trim.relW, trim.relH);
      }
      outDoc.addPage(page);
    }
...
// PDF points → CSS pixel rect（既存トリムの再表示用）
...
function drawTrimOverlay() {
  ...
  if (!trimRect || trimRect.w < 2 || trimRect.h < 2) { dimsEl.textContent = ''; return; }
  const sx = overlay.width / overlay.offsetWidth;
  const sy = overlay.height / overlay.offsetHeight;
  const cx = trimRect.x * sx, cy = trimRect.y * sy;
  const cw = trimRect.w * sx, ch = trimRect.h * sy;
  ...
  dimsEl.textContent = `W: ${Math.round(trimRect.w * sx / 2)} pt  H: ${Math.round(trimRect.h * sy / 2)} pt`;
}

function getOverlayCssCoords(e) {
  const rect = document.getElementById('pdf-crop-overlay').getBoundingClientRect();
  return { x: e.clientX - rect.left, y: e.clientY - rect.top };
}

function getTargetPages(currentOrigPage) {
  if (document.getElementById('trim-target').value === 'all') return [...activePages];
  if (selectedPages.size > 0) return [...selectedPages].map(idx => activePages[idx]);
  return [currentOrigPage];
}

function applyTrim() {
  if (!trimPreviewInfo || !trimRect || trimRect.w < 4 || trimRect.h < 4) {
    toast('トリム範囲をドラッグで選択してください', true); return;
  }
  const overlay = document.getElementById('pdf-crop-overlay');
  const sx = overlay.width / overlay.offsetWidth;
  const sy = overlay.height / overlay.offsetHeight;
  ...
  const relY = (overlay.height - trimRect.y * sy - trimRect.h * sy) / 2; // Y軸反転
...
cropOverlay.addEventListener('mousemove', e => {
  ...
  const w = Math.min(Math.abs(pos.x - trimDragStart.x), overlay.offsetWidth - x);
  const h = Math.min(Math.abs(pos.y - trimDragStart.y), overlay.offsetHeight - y);
  trimRect = { x, y, w, h };
  drawTrimOverlay();
});
...
cropOverlay.addEventListener('mouseleave', () => { trimDragging = false; });
